 ...
 */
/**
 * Compagnon de [EpgRepository] : message renvoyé par [EpgRepository.getOrLoad]/[EpgRepository.refresh]
 * tant que l'EPG est désactivé (voir la doc de classe, `epgDisabled`). Formulé comme un état
 * intentionnel plutôt qu'une panne — c'est la même chaîne qui remonte dans
 * `SettingsUiState.epgRefreshError` (Réglages, bouton "Rafraîchir l'EPG").
 */
private val DISABLED_RESULT =
    EpgLoadResult.Unavailable("Guide EPG désactivé — les chaînes se chargent normalement, seul le guide des programmes est coupé.")
...
class EpgRepository(context: Context) {
    ...
    /**
     * Fix (25 juillet 2026, vague 1 "stop crash", diagnostic point 1) : EPG désactivé à la
     * racine, à la demande explicite de l'utilisateur — plus besoin de cette fonctionnalité,
     * et c'est le seul moyen de garantir qu'AUCUN chemin ne peut plus jamais provoquer le
     * crash mémoire (fenêtre de rétention comprise), y compris un futur appelant qui
     * oublierait de la respecter.
     *
     * Avant ce correctif, seule la fenêtre de rétention (voir [EpgXmlParser]) bornait la
     * mémoire — mais `SettingsViewModel.refreshEpg` ("Rafraîchir l'EPG" de Réglages)
     * continuait de télécharger et parser l'intégralité de la source EPG effective à chaque
     * appui, fenêtrée ou non : sur un panel Xtream à 11 000+ chaînes, même le
     * téléchargement seul (potentiellement plusieurs dizaines de Mo) reste un pic mémoire
     * évitable si la fonctionnalité ne sert plus à rien.
     *
     * [EPG_DISABLED] coupe [getOrLoad]/[refresh] avant toute I/O ou tout parsing : ni
     * [loadRawBytes] ni [EpgXmlParser.parse] ne sont jamais atteints tant qu'il vaut `true`.
     * Rien d'autre dans l'app ne dépend de l'EPG pour fonctionner : le chargement des
     * chaînes (`ChannelRepository`/`PlaylistRepository`, M3U/Xtream) est un chemin
     * entièrement séparé, et les deux seuls autres appelants (`HomeViewModel
     * .loadPreviewProgramTitle`, l'OSD de `PlayerScreen`) étaient déjà désactivés côté
     * appelant depuis le même jour (voir leurs README respectifs) — la lecture d'une
     * chaîne n'a donc jamais dépendu de ce repository. Pour réactiver un jour : repasser
     * [EPG_DISABLED] à `false` (le mécanisme de fenêtrage/mutex/cache reste intact
     * ci-dessous, prêt à resservir tel quel).
     */
    private val epgDisabled = true
    ...
    // Instancié paresseusement (by lazy) : tant qu'epgDisabled vaut true, ce client OkHttp
    // dédié n'a aucune raison d'exister en mémoire.
    private val httpClient by lazy { com.dpflix.android.network.IptvHttpDataSourceFactory.httpClient() }

    /** Un seul résultat en cache par playlist — pas de vraie concurrence attendue dans
     *  l'archi actuelle (tout appelant passe par `viewModelScope`/Main, voir la doc de
     *  classe), mais [ConcurrentHashMap] plutôt qu'une `mutableMapOf` simple (fix
     *  25 juillet 2026, vague 1 "stop crash", diagnostic point 4) : un futur appelant qui
     *  contournerait ce chemin (ex. un Worker/Service en arrière-plan) ne provoquerait plus
     *  de corruption interne de la map elle-même. Coût nul en pratique (aucune contention
     *  réelle aujourd'hui), simple garde-fou défensif. */
    private val cache = ConcurrentHashMap<String, EpgLoadResult>()
     ...
     *  via [ConcurrentHashMap.computeIfAbsent] (fix point 4, même raison que [cache]
     *  ci-dessus) plutôt que par playlist connue à l'avance : cette classe n'a aucune
     *  visibilité sur la liste des playlists existantes (ce n'est pas son rôle).
     *  `computeIfAbsent` garantit qu'un seul [Mutex] est créé par playlist même si deux
     *  appelants demandent le même [playlistId] au même instant — l'ancien
     *  `getOrPut` sur `mutableMapOf` n'offrait aucune garantie de ce type (deux threads
     *  auraient pu chacun créer et poser leur propre [Mutex], ce qui aurait cassé la
     *  sérialisation que cette map existe justement pour assurer). */
    private val loadMutexes = ConcurrentHashMap<String, Mutex>()

    private fun mutexFor(playlistId: String): Mutex =
        loadMutexes.computeIfAbsent(playlistId) { Mutex() }
    ...
    suspend fun getOrLoad(playlist: Playlist): EpgLoadResult {
        if (epgDisabled) return DISABLED_RESULT
